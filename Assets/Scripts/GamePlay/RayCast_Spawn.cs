...
public class RayCast_Spawn : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            time = SpawnInterval;
            EnemiesSpawned.Add(Instantiate(EnemyPrefabToInstantiate, new Vector3(Random.Range(-RanMaxX, RanMaxX),  Random.Range(-RanMaxY, RanMaxY),Random.Range(0,RanMaxZ)),transform.rotation));
            EnemiesSpawned[count].transform.LookAt(target);
            Vector3 To_target = target.transform.position - EnemiesSpawned[count].transform.position;
        ...
        Debug.Log(EnemiesSpawned.Count + " ducks ");
